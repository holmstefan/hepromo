 ...
 *
 */
public class SimpleForwarderEnergieholz2018Test extends AbstractSimpleModelTest {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenMaschinist_proM3iR(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	protected double[] getErgebnis(double[] input) {
		...
		SimpleForwarderEnergieholz2018 model = new SimpleForwarderEnergieholz2018();
		int currentLineIndex = -1;
		if (input[++currentLineIndex] != -1) model.setEnergieholzmenge_m3iR(		input[currentLineIndex]	);
		...
		if (input[++currentLineIndex] != -1) model.setZopfdurchmesser_Rundholz_cm(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setEnergieholzanfall_m3iRproHa(	input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getKostenMaschinist_proM3iR();
