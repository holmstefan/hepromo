 ...
 *
 */
public class SimpleRadharvester2014Test extends AbstractSimpleModelTest {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkZeitRadharvester_PMH15(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenRadharvester_proM3oR(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	protected double[] getErgebnis(double[] input) {
		...
		SimpleRadharvester2014 model = new SimpleRadharvester2014();
		...
		if (input[++currentLineIndex] != -1) model.setFoermigkeit_Kategorie(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setAnteilLaubholz_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setLiegendesHolz_Kategorie(		input[currentLineIndex]	);
		...
		if (input[++currentLineIndex] != -1) model.setKostenRadharvester_proH(		input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getZeitRadharvester_PMH15();
		result[++currentLineIndex] = model.getKostenMaschinist_proM3oR();
		result[++currentLineIndex] = model.getKostenRadharvester_proM3oR();
