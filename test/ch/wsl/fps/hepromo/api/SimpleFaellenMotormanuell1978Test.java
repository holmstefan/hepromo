 ...
 *
 */
public class SimpleFaellenMotormanuell1978Test extends AbstractSimpleModelTest {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenPersonal_proM3(String testcaseName, double[] ergebnis, double[] expectedValues) {
		assertEqualsDynamicDelta(ergebnis[2], expectedValues[2]);
	}

	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenMotorsaege_proM3(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	protected double[] getErgebnis(double[] input) {
		...
		if (input[++currentLineIndex] != -1) model.setMassenmittelstamm_m3(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setBaumartengruppe_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setAnteilFaellenMitHandseilzug_Prz(	input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getKostenPersonal_proM3();
		result[++currentLineIndex] = model.getKostenMotorsaege_proM3();
