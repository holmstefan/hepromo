 ...
 *
 */
public class SimpleHelikopter2003Test extends AbstractSimpleModelTest {

	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkZeitHelikopter_PMH15(String testcaseName, double[] ergebnis, double[] expectedValues) {
		assertEqualsDynamicDelta(ergebnis[0], expectedValues[0]);
	}

	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkZeitPersonalForstbetrieb_WPPH(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenHelikopter_proM3oR(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	@Test
	public void checkLastvolumenAutomatischAnpassen() {
		SimpleHelikopter2003 model = new SimpleHelikopter2003();
		assertEqualsDynamicDelta(model.getZeitHelikopter_PMH15(), 4.48);
		
		model.setLastVolumenAutomatischBerechnen_Kategorie(2);
		model.setLastvolumen_m3(3);
		assertEqualsDynamicDelta(model.getZeitHelikopter_PMH15(), 4.27);
		
		model.setLastVolumenAutomatischBerechnen_Kategorie(1);
		assertEqualsDynamicDelta(model.getZeitHelikopter_PMH15(), 4.48);
	}
	...
	protected double[] getErgebnis(double[] input) {
		...
		if (input[++currentLineIndex] != -1) model.setArbeitsverfahren_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setHolztyp_Kategorie(				input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setBaumartengruppe_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setHorizontaldistanz_m(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setVertikaldistanzAbwaerts_m(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenHelikopter_proMin(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setAnflugpauschale_Betrag(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setHelikopterklasse_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setLastVolumenAutomatischBerechnen_Kategorie(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setLastvolumen_m3(					input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setAnzahlPersonalBeimHolzFliegen_Anzahl(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenPersonalProPerson_proH(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenMotorsaegenProMotorsaege_proH(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenKranfahrzeug_proH(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKalkulationInklLagerplatzarbeit_Kategorie(	input[currentLineIndex]	);
		...
		currentLineIndex = -1;
		result[++currentLineIndex] = model.getZeitHelikopter_PMH15();
		result[++currentLineIndex] = model.getZeitPersonalForstbetrieb_WPPH();
		result[++currentLineIndex] = model.getZeitMotorsaege_PMH15();
		result[++currentLineIndex] = model.getZeitKranfahrzeug_PMH15();
		result[++currentLineIndex] = model.getKostenHelikopter_proM3oR();
