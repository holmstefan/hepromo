...
import ch.wsl.fps.hepromo.api.SimpleForwarderEnergieholz2018Test;
 ...
 *
 */
public class SimpleForwarderEnergyWood2018Test extends SimpleForwarderEnergieholz2018Test {
	...
	protected double[] getErgebnis(double[] input) {
		// set input variables
		SimpleForwarderEnergyWood2018 model = new SimpleForwarderEnergyWood2018();
		int currentLineIndex = -1;
		if (input[++currentLineIndex] != -1) model.setEnergyWoodVolume_m3ob(		input[currentLineIndex]	);
		...
		if (input[++currentLineIndex] != -1) model.setTopDiameterRoundwood_cm(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setEnergyWoodVolume_m3obPerHa(	input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getCostsMachineOperator_perM3ob();
		result[++currentLineIndex] = model.getCostsForwarder_perM3ob();
		result[++currentLineIndex] = model.getCostsTransfer_perM3ob();
		result[++currentLineIndex] = model.getCostsFurtherWork_perM3ob();
		result[++currentLineIndex] = model.getCostsTotal_perM3ob();
