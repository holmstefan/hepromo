 ...
 *
 */
public class SimpleSchlepper2014Test extends AbstractSimpleModelTest {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenSchlepper_proM3oR(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	protected double[] getErgebnis(double[] input) {
		...
		SimpleSchlepper2014 model = new SimpleSchlepper2014();
		int currentLineIndex = -1;
		if (input[++currentLineIndex] != -1) model.setHolzmenge_m3iR(					input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setMittlererStueckinhalt_m3iRProStk(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setMittlereFahrentfernung_m(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setBeizugsdistanz_Kategorie(		input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getKostenSchlepper_proM3oR();
