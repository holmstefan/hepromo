 ...
 *
 */
public class SimpleMobilseilkran1999Test extends AbstractSimpleModelTest {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkZeitMobilseilkran_PMH15(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	@SuppressWarnings("unused")
	@Test(dataProvider="csvData")
	public void checkKostenMobilseilkran_proM3oR(String testcaseName, double[] ergebnis, double[] expectedValues) {
	...
	protected double[] getErgebnis(double[] input) {
		...
		SimpleMobilseilkran1999 model = new SimpleMobilseilkran1999();
		...
		if (input[++currentLineIndex] != -1) model.setSchwierigkeitSeitlicherZuzug_Kategorie(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setSeilsystem_Kategorie(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setMaschinenstandort_Kategorie(	input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setAnzahlStuetzen_Anzahl(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setEndmastVorhanden_Kategorie(		input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setTragseilhoeheEndmast_m(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenPersonal_proH(			input[currentLineIndex]	);
		if (input[++currentLineIndex] != -1) model.setKostenMobilseilkran_proH(		input[currentLineIndex]	);
		...
		result[++currentLineIndex] = model.getZeitMobilseilkran_PMH15();
		result[++currentLineIndex] = model.getZeitKranfahrzeug_PMH15();
		result[++currentLineIndex] = model.getKostenPersonal_proM3oR();
		result[++currentLineIndex] = model.getKostenMobilseilkran_proM3oR();
